...
- On `--continue`: restored results whose assembly has since been rebuilt are shown as **`Stale`** (dim tick/cross variant), making "these were the results, but the code changed" honest. A run replaces staleness with reality. *(Phase 6 note: run-materialised theory case rows cannot be restored until a rerun re-materialises them — they drop with a count in the restore notice; acceptable rough edge, revisit only if users notice.)*
...
- **Phase 5 — Watch mode. ✅ COMPLETE — all ACs closed.** (AC1–4/6–10 PASS. **AC5 CLOSED by owner measurement on representative dev hardware (WSL/ext4): T0→T4 median 1,735 ms / p95 2,822 ms vs the 4,000 ms bar — PASS with 2.3× headroom; build segment 827 ms confirms the sandbox's 6.26 s was a filesystem artifact, not a design problem. T0→T5 (save-to-green) ≈ 2.4 s median. POC-6's conditional is formally closed.** The `watch-latency` CI job remains as the regression guard; if a small hosted runner exceeds the bar, that is runner calibration, not an AC5 failure. M8 resolved outcome (b): NUnit 4.6.1 MTP host crashes on startup (exit 134, 'adapter not registered') before any handshake — a packaging regression vs POC-3's pass; §6.2 smoke validation catches it by design; upstream issue text in phase-5 notes. Deviations: `TTR_WATCH_LOG` for cycle timing; `--no-restore` on source-edit rebuilds; latency harness measures on /tmp to strip the sandbox FS artifact.) *Scope:* `--watch` (source mode, default) and `--watch external` per §9, debounce/quiescence/coalescing, changed-project-only re-evaluation, ProjectGraph dependents, auto-rerun with filter narrowing. *Exit criterion:* **measured** end-to-end single-file-change cycle < 4 s median on the POC-6 fixture with parallel builds — closes POC-6's conditional AC2. **Review:** edit-save-watch loop on the fixture; latency numbers in the PR description.

- **Phase 6 — Sessions (`--continue`). ✅ COMPLETE** (all AC PASS; POC-8 bars beaten by wide margins — save 31 ms, restore 47 ms, lazy detail 6–10 ms, footprint 2.37 MB, jitter p95 5.9 ms, kill -9 5/5; 202/203 tests, the 1 being a known /tmp-only Verify path-scrubber artifact carried to Phase 7 cleanup. Deviations: `finishedUtc` = save time (reducer purity; ≈ run-finish, sufficient for staleness); run-materialised theory rows drop on restore until a rerun re-materialises them, counted in the restore notice; perf/crash suites serialised into the integration collection; env-gated `TTR_CRASH_SAVE_DIR` test hook.) *Scope:* §10 persistence (state.json + packed JSONL sidecars, atomic writes, pruning), `Stale` presentation, target-hash mismatch degradation, save-after-run + on-exit. **Review:** run, quit, `ttr --continue`, verify restored tree/filters/results and Stale marking after a rebuild.

- **Phase 7 — Packaging & polish. ✅ CODE-COMPLETE (pending two owner inputs).** (`--tfm` single-TFM filter + unified `--log` umbrella (SESSION/BUILD/ADAPTER/WATCH categories) + exit-code sweep (0/1/2/3/130, VT/dumb refusal → 2, build-fail-preventing-any-run → 3); capability detection + ASCII glyph fallback + NO_COLOR/colourless stale-`~` + `TTR_ASCII`, refuse on dumb/no-VT; `PackAsTool` global-tool packaging (1.0.0, MIT, README packed) + CI `pack`/`install-test`(Linux+macOS)/`release`-on-tag; clean-machine install test re-proves POC-1 Strategy A from the installed path; scrubber-flake fixed at source (deterministic build-failure path); README + dev-notes + phase-8-backlog; help overlay ↔ README keymap parity. **Owner inputs:** (1) product name — owner chose to KEEP `ttr` (M4 = grep-verified no-op); (2) Windows Terminal + conhost manual smoke — script in `docs/windows-smoke.md`, result pending. Deviations + AC verdicts in `docs/phase-7-notes.md`.) *Scope:* dotnet global tool packaging; ASCII glyph fallback + capability detection; `--log` diagnostics; `--no-build`/`--tfm`/`--state-dir` flags finalised; README + demo cast; final rename from `ttr` if a product name has landed (§16.1). **Review:** `dotnet tool install` from a local feed on a clean machine; follow the README cold.
...
6. ~~Watch-cycle latency bar~~ **RESOLVED (owner measurement):** median 1,735 ms on representative dev hardware — bar holds unchanged; sandbox overage was a filesystem artifact. Residual note: calibrate the hosted-runner threshold separately if CI hardware proves slower than the bar (that would be runner calibration, not a product regression).
7. **(From Phase 5 M8)** NUnit 4.6.1 MTP hosting is broken upstream (exit 134 pre-handshake, regression vs POC-3's pass); ttr's smoke validation surfaces it correctly. File the prepared upstream issue (text in phase-5 notes) and re-test when NUnit/adapter ships a fix.
8. *(From POC-7)* Solution parsing at enterprise scale (100+ projects), Unicode project/folder names, and cross-folder duplicate project names were not exercised — re-validate with a large fixture before Phase 5 finalises watch-mode reload assumptions (expected fine given sub-ms warm loads at small scale, but unverified).
