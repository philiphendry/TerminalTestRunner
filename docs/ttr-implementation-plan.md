...
- **Phase 6 — Sessions (`--continue`). ✅ COMPLETE — all ACs closed.** *Scope:* §10 persistence (state.json + packed JSONL sidecars via `SessionStore`, atomic temp-file+rename writes, prune-to-3), `Stale` presentation (dim ✓/✗ overlay, orthogonal to outcome), target-fingerprint / schema / corrupt graceful degrade, save-after-every-run + on-exit (background snapshot), lazy restored-detail load + 'o'. All POC-8 bars met with margin (save 31 / restore 47 / lazy 6 ms; footprint 2.4 MB; jitter p95 5.9 ms; kill -9 5/5) — CI-asserted in `SessionPerfTests`/`SessionCrashSafetyTests`. Verified end-to-end in tmux (AC1 restore, AC2 rebuild→Stale→rerun-clears, AC3 lazy detail + 'o' on a restored failure). Deviations in `docs/phase-6-notes.md` (per-test finishedUtc = save time; run-materialised theory rows drop on restore until rerun; test-parallelism serialised). **Review:** run, quit, `ttr --continue`, verify restored tree/filters/results and Stale marking after a rebuild.
